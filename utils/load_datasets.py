...
from utils.wiki_loader_sentences import WikipediaDataSet
...
def load_dataset(dataset, 
    ...
    if dataset == 'BBC':
        ...
        for article in train:
            ...
            train_d.append([sents_list, labs, topics])
        ...
        for article in valid:
            ...
            valid_d.append([sents_list, labs, topics])
        ...
        for article in test:
            ...
            test_d.append([sents_list, labs, topics])
    ...
    elif dataset == 'wikisection_de_city':
        with open('data/wikisection/wikisection_de_city_train.json', encoding = 'utf-8') as f:
            train = json.load(f)
        ...
        with open('data/wikisection/wikisection_de_city_validation.json', encoding = 'utf-8') as f:
            valid = json.load(f)
        ...
        with open('data/wikisection/wikisection_de_city_test.json', encoding = 'utf-8') as f:
            test = json.load(f)
    ...
    elif dataset == 'wikisection_de_disease':
        with open('data/wikisection/wikisection_de_disease_train.json', encoding = 'utf-8') as f:
            train = json.load(f)
        ...
        for article in train:
            ...
            max_sents = 150 
            # if mask_inner_sentences else 10000
            if sents_list:
                train_d.append([sents_list[:max_sents], labs, topics])
        ...
        with open('data/wikisection/wikisection_de_disease_validation.json', encoding = 'utf-8') as f:
            valid = json.load(f)
        ...
        with open('data/wikisection/wikisection_de_disease_test.json', encoding = 'utf-8') as f:
            test = json.load(f)
        ...
        data = []
        data_path = './data/AudioBBC/modconhack_20210604/data_old'
        with open("data/AudioBBC/RadioNews_split.json") as f:
            split = json.load(f)
        
        train_split = [file_name[:-4]+".json" for file_name in split["train"]]
        valid_split = [file_name[:-4]+".json" for file_name in split["validation"]]
        test_split = [file_name[:-4]+".json" for file_name in split["test"]]
        ...
        for file in valid_split:
            ...
            for w in test_d:
              if w[0]:
                valid_data.append((w[0], expand_label(w[1], w[0])))
    ...
    elif dataset == 'NonNewsAudio':
        data = []
        data_path = './data/NonNewsAudio/data'
        train = []
        test = []
        valid = []
        
        with open('./data/NonNewsAudio/NonNews_split.json') as f:
            split = json.load(f)
            
        with open('./data/NonNewsAudio/map_nonnews_names.json') as f:
            mapper = json.load(f)
        ...
        for root, directories, files in os.walk(data_path):
            ...
            for file in files:
                if file[-4:] == 'json':
                    mapper_key = f"data/non_news_2021-07-05/audio\{file[:-5]}.mp3"
                    with open(os.path.join(root, file), 'rb') as f:
                        print(file)
                        ...
                        if mapper[mapper_key]+".npy" in split["train"]:
                            train.append((sents_list, expand_label(labs, sents_list), mapper[mapper_key]))
                        elif mapper[mapper_key]+".npy" in split["test"]:
                            test.append((sents_list, expand_label(labs, sents_list), mapper[mapper_key]))
                        elif mapper[mapper_key]+".npy" in split["validation"]:
                            valid.append((sents_list, expand_label(labs, sents_list), mapper[mapper_key]))
                        else:
                            raise ValueError(f"File {mapper_key} not found in split")
                        # test_d.append([sents_list, labs, file])
    ...
    elif dataset == 'wikisection_en_disease_topic_cls':
        with open('data/wikisection/wikisection_en_disease_train.json', encoding = 'utf-8') as f:
            train = json.load(f)
        ...
        for article in train:
            ...
            for segment in article['annotations']:
                ...
                topic = segment["sectionLabel"]
                if topic not in label_voc:
                    label_voc[topic] = len(label_voc)
                topic = label_voc[topic]
                sentences = nltk.sent_tokenize(article["text"][begin:end])
                sentences = [s for s in sentences if len(s) > 0 and s != "\n" and s!= "***LIST***."]
                if not sentences:
                    continue
                ...
                if delete_last_sentence:
                    sentences = sentences[:-1]
                sents_list.extend(sentences)
                labs.extend([topic for x in range(len(sentences))])
            
            # max_sents = 150 
            max_sents = 150 if mask_inner_sentences else 10000
            if sents_list:
                train_data.append([sents_list[:max_sents], labs, None])
            
        del(train) # clearing up space
        ...
        for article in valid:
            ...
            for segment in article['annotations']:
                ...
                topic = segment["sectionLabel"]
                if topic not in label_voc:
                    raise ValueError("Label in validation not found in training!")
                topic = label_voc[topic]
                sentences = nltk.sent_tokenize(article["text"][begin:end])
                sentences = [s for s in sentences if len(s) > 0 and s != "\n" and s!= "***LIST***."]
                if not sentences:
            ...
            max_sents = 150 if mask_inner_sentences else 10000
            if sents_list:
                valid_data.append([sents_list[:max_sents], labs, None])
        ...
        for article in test:
            ...
            for segment in article['annotations']:
                ...
                topic = segment["sectionLabel"]
                if topic not in label_voc:
                    raise ValueError("Label in test not found in training!")
                topic = label_voc[topic]
                sentences = nltk.sent_tokenize(article["text"][begin:end])
                sentences = [s for s in sentences if len(s) > 0 and s != "\n" and s!= "***LIST***."]
                if not sentences:
            ...
            max_sents = 150 if mask_inner_sentences else 10000
            if sents_list:
                test_data.append([sents_list[:max_sents], labs, topics])
