...
def compute_Pk(boundaries, ground_truth, window_size = None, boundary_symb = '1'):
    ...
    h = get_boundary_count(boundaries)
    t = get_boundary_count(ground_truth)
...
"""
Below function should be used for comparison with TxtSeg_MTL, as these are the ones they used
"""


def get_boundary_count(data, label_num=1, msn=0):
    '''
    param:msn - maximum sentence number
    param:label_num - text segment 기준 ( 0  or 1 )
    '''
    result = []
    idx=0
    for d in data:
